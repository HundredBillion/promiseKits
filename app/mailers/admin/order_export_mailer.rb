...
module Admin
  class OrderExportMailer < ApplicationMailer
    ...
    def export_email(recipient:, file_path:, filename:, export_id: nil, scheduled_for: nil)
      ...
      def export_email(recipient:, file_path:, filename:, export_id: nil, scheduled_for: nil, exported_count: nil)
        ...
        scheduled_for_utc = scheduled_for.present? ? scheduled_for.to_time.utc : Time.current.utc

        # Determine window label using centralized config helper so formatting is consistent
        window_label = OrderExport::Config.window_label_for(scheduled_for_utc)
